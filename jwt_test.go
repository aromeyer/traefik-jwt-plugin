...
func TestServeHTTPJwtAlwaysPresent(t *testing.T) {
	cfg := traefik_jwt_plugin.CreateConfig()
	cfg.PayloadFields = []string{"exp"}
	cfg.JwtAlwaysPresent = true
	cfg.Keys = []string{"-----BEGIN PUBLIC KEY-----\\nMwIDAQAB\n-----END PUBLIC KEY-----"}
	ctx := context.Background()
	nextCalled := false
	next := http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) { nextCalled = true })
	...
	if recorder.Result().StatusCode != http.StatusForbidden {
		t.Fatalf("Expected status %d, received %d", http.StatusForbidden, recorder.Result().StatusCode)
	}
	body, err := io.ReadAll(recorder.Result().Body)
	if err != nil {
		t.Fatalf("Error will getting response body - err: %s", err.Error())
	}
	must_contain := "Authorization header with JWT Token is required"
	if !strings.Contains(string(body), must_contain) {
		t.Fatalf("Expected response must contain '%s' , received: %s", must_contain, body)
	}
}

func TestServeHTTPJWTExpiryCheck(t *testing.T) {
	cfg := traefik_jwt_plugin.CreateConfig()
	cfg.PayloadFields = []string{"exp"}
	cfg.Required = true
	cfg.JwtExpiryCheck = true
	cfg.Keys = []string{"-----BEGIN PUBLIC KEY-----\\nMwIDAQAB\n-----END PUBLIC KEY-----"}
	ctx := context.Background()
	nextCalled := false
	next := http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) { nextCalled = true })
	...
	must_contain := "Token has expired"
	if !strings.Contains(string(body), must_contain) {
		t.Fatalf("Expected response must contain '%s' , received: %s", must_contain, body)
	}
}
